...
import "./ApprovedContractsTable.css"
import { Image, ProgressBar } from "react-bootstrap"
import { BsThreeDotsVertical } from "react-icons/bs"
import { useDispatch } from "react-redux"
import { formatNumbers } from "../../../../../services/formatNumbers"
import { adminGetOneAnalysis, adminGetSimilarContracts, adminMarkScam, adminGetListContract, adminUnCheckScam, adminDeleteCompleteContract } from "../../../../../store/actions/adminActions"
import gradiantImg from "../../../../../images/LogoDisabled.png"
import PopupState, { bindMenu, bindTrigger } from "material-ui-popup-state"
import Menu from "@mui/material/Menu"
import MenuItem from "@mui/material/MenuItem"

function ApprovedContractsTable({ name, img, scam, marketCap, difVolume, segurityPoints, num, setShowModal, address, setContractAddress, disabledButtom, setDisabledButtom }) {
  const dispatch = useDispatch()
  const handleShow = async () => {
    setDisabledButtom(true), await dispatch(adminGetOneAnalysis(address)), setContractAddress(address), setShowModal(true), setDisabledButtom(false)
  }

  const handleMarkScam = async () => {
    setDisabledButtom(true), dispatch(adminMarkScam(address)), dispatch(adminGetListContract("analyzed")), alert("Contract marked as scam"), setDisabledButtom(false)
  }

  const handleUnCheckScam = async () => {
    setDisabledButtom(true), dispatch(adminUnCheckScam(address)), dispatch(adminGetListContract("analyzed")), alert("Contract unmarked as scam"), setDisabledButtom(false)
  }
  const handleDelete = async () => {
    await dispatch(adminDeleteCompleteContract(address))
  ...
  return (
    <tr>
      ...
      <td>
        <div>
          <Image
            src={img === "https://ethplorer.io/undefined" ? gradiantImg : img || gradiantImg}
            onError={({ currentTarget }) => {
              currentTarget.onerror = null // prevents looping
              currentTarget.src = gradiantImg
            }}
            width="28px"
            height="28px"
            fluid
      ...
      <td>
        <div className="ApprovedContractsTable_Td_ButtonCheck">
          {/* boton "X" */}
          <button type="button" disabled={disabledButtom || scam} style={scam ? { cursor: "no-drop", opacity: 0.4 } : null} onClick={() => handleMarkScam()}>
      ...
      <td>
        <div className="ApprovedContractsTable_Td_ButtonCheck">
          {/* boton "check" */}
          <button type="button" disabled={disabledButtom || !scam} style={!scam ? { cursor: "no-drop", opacity: 0.4 } : null} onClick={() => handleUnCheckScam()}>
